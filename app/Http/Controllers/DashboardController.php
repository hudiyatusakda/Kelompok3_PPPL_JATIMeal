...
class DashboardController extends Controller
{
    ...
    // Fungsi Helper untuk mengubah string "10k-15k" menjadi angka [10000, 15000]
    private function parsePriceRange($rangeString)
    {
        // Bersihkan string (hapus "Rp", titik, spasi)
        $clean = str_replace(['Rp', '.', ' '], '', $rangeString);

        // Cek format "<10k"
        if (str_contains($clean, '<')) {
            $val = (int) filter_var($clean, FILTER_SANITIZE_NUMBER_INT);
            return ['min' => 0, 'max' => $val * 1000]; // Asumsi "k" = 000
        }
